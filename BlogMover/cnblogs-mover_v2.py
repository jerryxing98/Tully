...
entryId = 10000
blogApp = 'jerryxing'
posturl = u'http://www.cnblogs.com/mvc/blog/BlogPostInfo.aspx'
prexurl = u'http://www.cnblogs.com/post/prevnext'

def test_exist(recs):
    '''
    测试记录是否相同的存在
    '''
    sql_cmd=u"select * from blog_article where title = ? " 
    print sql_cmd
    cu.execute(sql_cmd,[recs['title']])
    res=cu.fetchall()
    if len(res)>0:
        return True
    else:
        return False
def get_category(recs):
     '''
     测试种类是否存在
     '''
     sql_cmd=u'''select * from blog_category where
     name= '%s' ''' % recs['category']
     cu.execute(sql_cmd)
     res=cu.fetchall()
     if len(res)>0:
         return res[0][0]
         ...
         seq=cu.fetchall()[0][0]
         id=seq+1
         sql_cmd=u"insert into blog_category (id,name,slug) values (%d,'%s','%d')" % (id,recs['category'],id)
         print sql_cmd
         cu.execute(sql_cmd)
         cx.commit()
         return id
    ...
    for cnt in cnts:
        ...
        if len(recs) == len(heads):
            ...
            if test_exist(test_recs):
                print('Exist!')
            sql_cmd = u'insert into tab_day (%s) values (%s) ;' % (head, record)
            cu.execute(sql_cmd)
        else:
            print('error')
    cx.commit() # 提交事务
...
def fetchEntry(url,datetimePattern = '%Y-%m-%d %H:%M',mode='all'):
    ...
    #title
    temp = soup.findAll(attrs={"class":"postTitle2"})[0]
    if temp :
    ...
    #datetime
    temp = soup.find(id="post-date")
    if temp :
        i['datetime']=temp.string.strip()+':00'
        logging.debug(u"提取日期 %s",i['datetime'])
    else :
        logging.warning(u"无法提取日志发表时间")
        sys.exit(2)
    #content
    temp = soup.find(id='cnblogs_post_body')
    if temp :
    ...
    #使用ajax进行读取
    match = re.search(r"var allowComments=true,isLogined=false,cb_blogId=(.*?),cb_entryId=(.*?),cb_blogApp=currentBlogApp,cb_blogUserGuid='(.*?)'",page, re.DOTALL | re.IGNORECASE | re.MULTILINE)
    print match
    if match:
        try:
            if match.group(1) and match.group(2) and match.group(3) :
                blogId=match.group(1)
                postId=match.group(2)
                blogUserGuid=match.group(3)
                data = {'blogApp':blogApp,'blogId':blogId,'blogUserGuid':blogUserGuid,'postId':postId}
                sepage = post(posturl,data)
                logging.debug(u"下载分类信息成功，准备日志分类信息")
                soup2 =  BeautifulSoup(sepage)
                temp2 = soup2.findAll('a')[0]
                i['category'] = temp2.next
                logging.debug(u"提取分类 %s",i['category'])
                datas = {'postId':postId,'blogId':blogId,'dateCreated':i['datetime'].replace('-','/')}
                logging.debug(u"===============data================")
                sepage = get(prexurl,datas)
                logging.debug(u"下载下一个连接信息成功，准备连接信息")
                match = re.search(r'href="(.*?)"',sepage,re.DOTALL | re.IGNORECASE | re.MULTILINE)
                print match
                if match:
                    logging.debug(u"上一篇日志URL: %s",match.group(1))
                    i['PrevPermaSerial'] = match.group(1)
            else:
                logging.warning("无法提取内容")
        except:
            logging.debug(u"===============提取分配出错 堆栈信息================")
        finally:
            pass
    '''
    temp = soup.find(id="BlogPostCategory")
    if temp :
    ...
    '''
    '''
    temp = soup.findAll(attr={'class':'p_n_p_prefix'})
    if temp :
        i['PrevPermaSerial']=u''.join(temp[0].string.strip())
        logging.debug(u"提取上一篇成功");
    else:
        logging.warning(u"无法提取上一个链接")
    '''
    #comments
    #不对评论进行提取
    '''
...
def main():
    ...
    parser = OptionParser()
    parser.add_option("-s","--source",action="store", type="string",dest="srcURL",
    help="source msn/live space address")
    parser.add_option("-f","--startfrom",action="store", type="string",dest="startfromURL",
    help="a permalink in source msn/live space address for starting with, if this is specified, srcURL will be ignored.")    
    parser.add_option("-x","--proxy",action="store",type="string",dest="proxy",
    help="http proxy server, only for connecting live space.I don't know how to add proxy for metaWeblog yet. So this option is probably not useful...")
    ...
    help="limit number of transfered posts, you can use this option to test")
    parser.add_option("-m","--mode",action="store",type="string",dest="mode",default="all",
    help="Working mode, 'all' or 'commentsOnly'. Default is 'all'. Set it to 'commentsOnly' if you have used earlier version of this script to move posts. Set it to 'postsOnly' if you can't upload the comments-post page to your dest WordPress blog so can't move comments")
    ...
    count = 0
    cacheFile = open('entries.cache','wb')
    try:
        while permalink:
